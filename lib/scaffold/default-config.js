...
/**
 * Will return the path and default bellcore-node configuration. It will search for the
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.bellcore');
  var defaultConfigFile = path.resolve(defaultPath, './bellcore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['bellcoind', 'web'];
