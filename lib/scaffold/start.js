...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bellcoind is necessary for this upgrade with the "reindex=1" bellcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and bellcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed bellcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bellcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in bellcore-node services directory
