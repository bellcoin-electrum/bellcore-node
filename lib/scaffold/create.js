...
var yespower = require('bellcoin-yespower');
...
var BASE_PACKAGE = {
  description: 'A full Bellcoin node build with Bellcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'bellcoin-yespower': yespower.version,
    'bellcore-lib': bitcore.version,
    'bellcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bellcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bellcore-node directory and configuration
